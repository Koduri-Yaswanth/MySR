...
import {
  callNearByNgo,
  callNearByNgoError,
  callNearByNgoSuccess,
} from '../slice/NearByNGO';
import {NearByNGOInfo} from '../../../types';

function* nearByNgo(action: PayloadAction<NearByNGOInfo>): SagaReturnType<any> {
  try {
...
export default function* watchNearByNgo() {
  yield takeLatest(callNearByNgo.type, nearByNgo);
}
