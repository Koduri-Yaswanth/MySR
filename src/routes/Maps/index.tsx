...
import PlusIcon from 'react-native-vector-icons/FontAwesome6';
...
import CustomPopupAddAddress from '../../components/CustomPopupAddAddress';
...
import {
  ...
  updateAddressInfo,
  updateAddressClear,
} from '../AddAddress/slice/updateAddress';
import {callGetAddress, clearGetAddress} from '../AddAddress/slice/getAddress';
import {
  callGetAddressByLatLong,
  getAddressByLatLongInfo,
} from '../AddAddress/slice/getAddressByLatLong';
import {
  callGetLatLongByAddress,
  getLatLongByAddressInfo,
} from '../AddAddress/slice/getLatLongByAddress';
import {
  callUpdateProfile,
  updateProfileInfo,
  clearUpdateProfile,
} from '../MyAccount/slice/updateProfile';
import {
  callGetProfile,
  getProfileInfo,
  clearProfile,
} from '../MyAccount/slice/getProfile';

import {clearGetAddressById} from '../MainScreen/slice/GetAddressById';
...
const Maps = (prop: any) => {
  ...
  const {getAddressByLatLongSuccess, isAddressLoading} = useAppSelector(
    getAddressByLatLongInfo,
  );
  ...
  const handlebackpress = ({RootNavigation}: any) => {
  ...
  const onGetAddressByLatLong = () => {
    ...
    dispatch(callGetAddressByLatLong(payload));
  };
  ...
  const getAddress = () => {
    ...
    dispatch(callGetAddress(payload));
  };
  ...
  useEffect(() => {
    if (getAddressByLatLongSuccess && getAddressByLatLongSuccess?.data) {
      setFirstLane(
        getAddressByLatLongSuccess?.data?.results[0]?.components?.road,
      );
      setLastLane(
        getAddressByLatLongSuccess?.data?.results[0]?.components?.neighbourhood,
      );
      setCity(getAddressByLatLongSuccess?.data?.results[0]?.components?.city);
      setState(getAddressByLatLongSuccess?.data?.results[0]?.components?.state);
      setZipCode(
        getAddressByLatLongSuccess?.data?.results[0]?.components?.postcode,
      );
      setCountry(
        getAddressByLatLongSuccess?.data?.results[0]?.components.country,
      );
      ...
      setCurrentAddress({
        firstLine:
          getAddressByLatLongSuccess?.data?.results[0]?.components?.road,
        lastLine:
          getAddressByLatLongSuccess?.data?.results[0]?.components
            ?.neighbourhood,
        city: getAddressByLatLongSuccess?.data?.results[0]?.components?.city,
        state: getAddressByLatLongSuccess?.data?.results[0]?.components?.state,
        zipCode:
          getAddressByLatLongSuccess?.data?.results[0]?.components?.postcode,
        country:
          getAddressByLatLongSuccess?.data?.results[0]?.components.country,
    ...
    if (
      getAddressByLatLongSuccess &&
      getAddressByLatLongSuccess.data.length > 0
    ) {
      const addressData =
        getAddressByLatLongSuccess?.data?.results[0]?.formatted;
      setFirstLane(addressData?.components?.road);
      setLastLane(addressData?.components?.neighbourhood);
      setCity(addressData?.components?.city);
      setCountry(addressData?.country);
      setZipCode(addressData?.components?.postcode);
      setState(addressData?.components?.state);

      setCurrentAddress({
        firstLine: addressData.road,
        lastLine: addressData?.components?.neighbourhood,
        city: addressData?.components?.city,
        state: addressData?.components?.state,
        zipCode: addressData?.components?.postcode,
                  ...
                  <>
                    ...
                    <Pressable
                      ...
                      <View style={styles.padding}>
                        <LocationIcon name="location-sharp" size={20} />
                    ...
                    <Pressable
                      ...
                      <View style={styles.padding}>
                        ...
                        <Text style={styles.textButton}>
                          {screenData?.data ? 'updateAddress' : 'Add Address'}
                        </Text>
                      </View>
                    </Pressable>
                  </>
                    ...
                    <View>
                      ...
                      {isAddressLoading ? (
                        ...
                        <Text style={styles.address}>
                          {
                            getAddressByLatLongSuccess?.data?.results[0]
                              ?.formatted
                          }
                        </Text>
                      )}
            ...
            <Modal
              ...
              <View style={styles.center}>
                <CustomPopupAddAddress
                  ...
                  title={screenData?.data ? 'Update Address' : 'Add Address'}
                  buttonTitle={'Save'}
                  closeModal={
